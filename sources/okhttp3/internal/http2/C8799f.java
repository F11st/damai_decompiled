...
final class C8799f implements Closeable {
    private static final Logger g = Logger.getLogger(C8793c.class.getName());
    private final BufferedSink a;
    private final boolean b;
    private final Buffer c;
    private int d;
    private boolean e;
    final C8790b.C8792b f;

    /* JADX INFO: Access modifiers changed from: package-private */
    public C8799f(BufferedSink bufferedSink, boolean z) {
        this.a = bufferedSink;
        this.b = z;
        Buffer buffer = new Buffer();
        this.c = buffer;
        this.f = new C8790b.C8792b(buffer);
        this.d = 16384;
    }
    ...
    public synchronized void a(C8802h c8802h) throws IOException {
        if (!this.e) {
            this.d = c8802h.f(this.d);
            if (c8802h.c() != -1) {
                this.f.e(c8802h.c());
            }
            c(0, 0, (byte) 4, (byte) 1);
    ...
    public void c(int i, int i2, byte b, byte b2) throws IOException {
        ...
        if (logger.isLoggable(Level.FINE)) {
            logger.fine(C8793c.b(false, i, i2, b, b2));
        }
        int i3 = this.d;
        if (i2 > i3) {
            throw C8793c.c("FRAME_SIZE_ERROR length > %d: %d", Integer.valueOf(i3), Integer.valueOf(i2));
        }
        if ((Integer.MIN_VALUE & i) != 0) {
            throw C8793c.c("reserved bit set: %s", Integer.valueOf(i));
        }
        i(this.a, i2);
    ...
    public synchronized void connectionPreface() throws IOException {
        if (!this.e) {
            if (this.b) {
                Logger logger = g;
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine(C8753a.q(">> CONNECTION %s", C8793c.a.hex()));
                }
                this.a.write(C8793c.a.toByteArray());
                this.a.flush();
                return;
            }
            return;
    ...
    public synchronized void d(int i, ErrorCode errorCode, byte[] bArr) throws IOException {
        if (!this.e) {
            if (errorCode.httpCode != -1) {
                ...
                throw C8793c.c("errorCode.httpCode == -1", new Object[0]);
            }
        } else {
            throw new IOException(IRequestConst.CLOSED);
        }
    ...
    public synchronized void e(boolean z, int i, List<C8789a> list) throws IOException {
        if (!this.e) {
            this.f.g(list);
    ...
    public synchronized void g(C8802h c8802h) throws IOException {
        if (!this.e) {
            int i = 0;
            c(0, c8802h.j() * 6, (byte) 4, (byte) 0);
            while (i < 10) {
                if (c8802h.g(i)) {
                    this.a.writeShort(i == 4 ? 3 : i == 7 ? 4 : i);
                    this.a.writeInt(c8802h.b(i));
                }
                i++;
            }
            this.a.flush();
    ...
    public synchronized void pushPromise(int i, int i2, List<C8789a> list) throws IOException {
        if (!this.e) {
            this.f.g(list);
            long size = this.c.size();
    ...
    public synchronized void windowUpdate(int i, long j) throws IOException {
        ...
        if (j != 0 && j <= 2147483647L) {
            ...
            throw C8793c.c("windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL: %s", Long.valueOf(j));
        }
    }
}
