package com.uc.webview.export.cyclone;

import android.content.Context;
import android.util.Base64;
import android.webkit.ValueCallback;
import java.io.File;
import java.io.FileOutputStream;
import java.util.HashSet;
import java.util.concurrent.ConcurrentLinkedQueue;

/* compiled from: Taobao */
@Constant
/* loaded from: classes11.dex */
public class UCLibrary {
    private static final String CLASS_IMPL = "com.uc.webview.export.internal.setup.UCLibraryLoaderImpl";
    private static final String DEX_FILE_DATA = "";
    private static final long DEX_FILE_LENGTH = 2695;
    private static final String DEX_FILE_NAME = "d_2695_24354940.jar";
    private static final long DEX_FILE_TIME = 24354940;
    private static final HashSet<String> sLoadedSOFiles = new HashSet<>(2);

    private UCLibrary() {
    }

    public static void load(Context context, String str, ClassLoader classLoader) {
        load(context, str, classLoader, null);
    }

    public static void load(Context context, String str, ClassLoader classLoader, ValueCallback<String>... valueCallbackArr) {
        FileOutputStream fileOutputStream;
        Throwable th;
        ValueCallback<String> valueCallback = UCCyclone.loadLibraryCallback;
        if (valueCallback != null) {
            valueCallback.onReceiveValue(str);
            return;
        }
        if (valueCallbackArr != null) {
            try {
                if (valueCallbackArr.length > 0 && valueCallbackArr[0] != null) {
                    valueCallbackArr[0].onReceiveValue(str);
                    return;
                }
            } catch (Throwable th2) {
                UCLogger create = UCLogger.create("e", "cyclone");
                if (create != null) {
                    create.print("load:" + str, th2);
                }
                File file = new File(UCCyclone.getDataFolder(context), DEX_FILE_NAME);
                if (!file.exists() || file.length() != DEX_FILE_LENGTH) {
                    try {
                        fileOutputStream = new FileOutputStream(file);
                        try {
                            fileOutputStream.write(Base64.decode(DEX_FILE_DATA, 2));
                        } catch (Throwable th3) {
                            th = th3;
                            try {
                                throw new UCKnownException((int) ErrorCode.LIBRARY_LOADER_DEX_FILE_CREATE_ERROR, th);
                            } finally {
                                UCCyclone.close(fileOutputStream);
                            }
                        }
                    } catch (Throwable th4) {
                        fileOutputStream = null;
                        th = th4;
                    }
                }
                if (file.exists() && file.length() == DEX_FILE_LENGTH) {
                    ConcurrentLinkedQueue<File> concurrentLinkedQueue = UCCyclone.sInusedFiles;
                    if (concurrentLinkedQueue != null) {
                        concurrentLinkedQueue.add(file);
                    }
                    try {
                        HashSet<String> hashSet = sLoadedSOFiles;
                        synchronized (hashSet) {
                            if (hashSet.contains(str)) {
                                return;
                            }
                            File file2 = new File(str);
                            String absolutePath = file.getAbsolutePath();
                            String parent = file.getParent();
                            String parent2 = file2.getParent();
                            if (classLoader == null) {
                                classLoader = UCLibrary.class.getClassLoader();
                            }
                            UCCyclone.invoke(Class.forName(CLASS_IMPL, false, new UCLoader(absolutePath, parent, parent2, classLoader)), "load", new Class[]{String.class, ValueCallback.class}, new Object[]{str, UCCyclone.statCallback});
                            hashSet.add(str);
                            return;
                        }
                    } catch (Throwable th5) {
                        throw new UCKnownException((int) ErrorCode.LIBRARY_LOADER_CLASS_NOT_FOUND, th5);
                    }
                }
                throw new UCKnownException((int) ErrorCode.LIBRARY_LOADER_DEX_FILE_MISSING, String.format("Library loader dex file [%s] missing.", file.getAbsolutePath()));
            }
        }
        if (classLoader != null && UCLibrary.class.getClassLoader() != classLoader) {
            throw new RuntimeException("systemLoadDelegate should be specified while loading so in non-cyclone module.");
        }
        System.load(str);
    }
}
