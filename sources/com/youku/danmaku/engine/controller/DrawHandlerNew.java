...
public class DrawHandlerNew extends Handler implements IDrawHandler {
    ...
    public class FrameCallback implements Choreographer.FrameCallback {
        ...
        public void doFrame(long j) {
            DrawHandlerNew.this.sendEmptyMessage(2);
        }
    }

    public DrawHandlerNew(Looper looper, IDanmakuViewController iDanmakuViewController, boolean z) {
        super(looper);
    ...
    private void prepare(final Runnable runnable) {
        if (this.drawTask == null) {
            this.drawTask = createDrawTask(this.mDanmakuView.isDanmakuDrawingCacheEnabled(), this.timer, this.mDanmakuView.getContext(), this.mDanmakuView.getWidth(), this.mDanmakuView.getHeight(), this.mDanmakuView.isHardwareAccelerated(), new IDrawTask.TaskListener() { // from class: com.youku.danmaku.engine.controller.DrawHandlerNew.3
                @Override // com.youku.danmaku.engine.controller.IDrawTask.TaskListener
                public void onDanmakuAdd(BaseDanmaku baseDanmaku) {
                    if (baseDanmaku.isTimeOut()) {
                        return;
                    }
                    long j = baseDanmaku.time - DrawHandlerNew.this.timer.currMillisecond;
                    if (j >= DrawHandlerNew.this.mContext.mDanmakuFactory.mMaxDanmakuDuration || (!DrawHandlerNew.this.mInWaitingState && !DrawHandlerNew.this.mRenderingState.nothingRendered)) {
                        if (j <= 0 || j > DrawHandlerNew.this.mContext.mDanmakuFactory.mMaxDanmakuDuration) {
                            return;
                        }
                        DrawHandlerNew.this.sendEmptyMessageDelayed(11, j);
                        return;
                    }
                    DrawHandlerNew.this.notifyRendering();
                }

                @Override // com.youku.danmaku.engine.controller.IDrawTask.TaskListener
                public void onDanmakuConfigChanged() {
                    DrawHandlerNew.this.redrawIfNeeded();
                }

                @Override // com.youku.danmaku.engine.controller.IDrawTask.TaskListener
                public void onDanmakuShown(BaseDanmaku baseDanmaku) {
                    if (DrawHandlerNew.this.mCallback != null) {
                        DrawHandlerNew.this.mCallback.danmakuShown(baseDanmaku);
                    }
                }
                ...
                public void onDanmakusDrawingFinished() {
                    if (DrawHandlerNew.this.mCallback != null) {
                        DrawHandlerNew.this.mCallback.drawingFinished();
                    }
                }

                @Override // com.youku.danmaku.engine.controller.IDrawTask.TaskListener
                public void ready() {
                    DrawHandlerNew.this.initRenderingConfigs();
                    runnable.run();
                }
            });
        } else {
    ...
    private void updateInChoreographer() {
        ...
        try {
            ...
            e.printStackTrace();
            if (Log.isDebug()) {
                Log.d("DrawHandlerNew", "Choreographer is error! use UPDATE_IN_NEW_THREAD");
            }
    ...
    private void updateInNewThread() {
        ...
        UpdateThread updateThread = new UpdateThread("DFM Update") { // from class: com.youku.danmaku.engine.controller.DrawHandlerNew.2
            @Override // com.youku.danmaku.engine.controller.UpdateThread, java.lang.Thread, java.lang.Runnable
            public void run() {
                long uptimeMillis = SystemClock.uptimeMillis();
                while (!isQuited() && !DrawHandlerNew.this.quitFlag) {
                    long uptimeMillis2 = SystemClock.uptimeMillis();
                    if (DrawHandlerNew.this.mFrameUpdateRate - (SystemClock.uptimeMillis() - uptimeMillis) <= 1) {
                        long syncTimer = DrawHandlerNew.this.syncTimer(uptimeMillis2);
                        if (syncTimer >= 0) {
                            long drawDanmakus = DrawHandlerNew.this.mDanmakuView.drawDanmakus();
                            if (drawDanmakus > DrawHandlerNew.this.mCordonTime2) {
                                DrawHandlerNew.this.timer.add(drawDanmakus);
                                DrawHandlerNew.this.mDrawTimes.clear();
                            }
                            if (!DrawHandlerNew.this.mDanmakusVisible) {
                                DrawHandlerNew.this.waitRendering(10000000L);
                            } else if (DrawHandlerNew.this.mRenderingState.nothingRendered && DrawHandlerNew.this.mIdleSleep) {
                                long j = DrawHandlerNew.this.mRenderingState.endTime - DrawHandlerNew.this.timer.currMillisecond;
                                if (j > 500) {
                                    DrawHandlerNew.this.notifyRendering();
                                    DrawHandlerNew.this.waitRendering(j - 10);
                                }
                            }
                        } else {
    ...
    */
    public void handleMessage(android.os.Message r17) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.youku.danmaku.engine.controller.DrawHandlerNew.handleMessage(android.os.Message):void");
    }
